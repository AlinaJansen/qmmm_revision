...
def write_mdp(mdpname, rcoulomb, rvdw, nbradius, inout, gmxplus = True):#gmxplus = True added by Nicola
    if rcoulomb == 0:
        rcoulomb = nbradius
    if rvdw == 0:
        rvdw = rcoulomb
...
def get_qmenergy(qmfile, qmmmInputs):
    ...
    if str(qmprog) == "ORCA": # Added by Nicola
        ...
        with open(str(qmfile + ".log")) as ifile:
            for line in ifile:
                match = re.search(r"^\s*LOEWDIN\s*ATOMIC\s*CHARGES", line, flags=re.MULTILINE) # or match = re.search(r"^\s*MULLIKEN\s*ATOMIC\s*CHARGES", line, flags=re.MULTILINE)
                if match:
                    for line in ifile:
                        break
                    for line in ifile:
