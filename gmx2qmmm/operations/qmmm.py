...
def get_full_coords_nm(gro):  # read g96
    '''
    input: g96 file
    return: list of coordinates of all atoms in nm
    '''
    fullcoords = []
...
def get_full_coords_angstrom(gro):
    '''
    input: g96 file
    return: list of coordinates of all atoms in angstrom
    '''
    fullcoords = []
    with open(gro) as ifile:
        count = 0
...
def get_atoms(qmmmtop, logfile):
    '''
    input: qmmmtop (some sort of topology file?) and logfile
    return: list of atom names
    -----------------
    apparently looks for atom masses in qmmmtop file and translates them to a list of atom names 
    '''
    atoms = []
    mass_map = {
        "H": "1.008",
...
def write_mdp(mdpname, nbradius, mminfo):
    ...
    input: mdpname; 
           ...
           mminfo: user defined coulomb and vdw cutoff radius
    returns: none
    ---------------
    writes the mdp file for gmx
    '''
    rcoulomb = mminfo[1]
    rvdw = mminfo[2]
    #for key,value in kwargs.items():
    #        print(key)
    #        if "rcoulomb" in str(key):
    #                rC = value
    #        if "rvdw" in str(key):
    #                rV = value
    if rcoulomb == 0:    
...
def get_nbradius(gro):
    '''
    input: g96 file
    return: maximum distance between any two atoms in the system in nm
    '''
...
def update_gro_box(gro, groname, nbradius, logfile):
    ...
    input: 
        gro: g96 file
        groname: name of new g96 file 
        nbradius: maximum radius between two atoms
        logfile
    returns: none
    -----------------
    writes a new g96 file with increased box size, margin 10*nbradius
    '''
...
def make_gmx_inp(
    jobname, gro, qmmmtop, qmatomlist, mminfo, curr_step, logfile, basedir, realprefix 
):
    '''
    input: 
        jobname
        gro: g96 file
        qmatomlist
        mminfo: for writing new mdp
        curr_step
        logfile
        basedir
        realprefix: gmx command for calling subprocess
    returns: name of new tpr
    --------------------------
    calls functions to write new mdp and g96 file
    creates new tpr file with gmx subprocess
    '''
    insert = ""
    if int(curr_step) != 0:
...
def make_g16_inp(
    ...
    input:
        ...
        logfile
        nmaflag
    returns: name of gaussian input file
    -----------------------
    creates the gaussian input file with QM atoms, link atoms and pcf
    '''
    insert = ""
    oldinsert = ""
    if int(curr_step) != 0:
...
def get_qmforces_au(qmatomlist, m1list, qmmmtop, qminfo, jobname, curr_step, logfile):
    ...
    input:
        ...
        logfile
    returns:
        list of force vectors for each QM atom and the m1 atoms in atomic units
    '''
    qmforces = []
    qmonlyforcelist = []
    pcf_grad = []
...
def get_mmforces_au(jobname, curr_step, logfile, pathinfo):
    ...
    input:
        ...
        curr_step
        logfile
        pathinfo: path information for gmx, Grolib and g16
    return:
        list of force vectors for all atoms in atomic units extracted from the gromacs trr file
    '''
    prefix = pathinfo[3]
...
def get_linkforces_au(
    ...
    '''
    input: 
        linkcorrlist: list of link atoms
        qmatomlist
        xyzq
        pcffile
        qm_corrdata
        ...
        qmmminfo
    return:
        list of force vectors on link atoms in aulist of force vectors on link atoms in au
    '''
    linkforces = []
...
def make_clean_force(total_force):
    '''
    input:
        total_force: some list of forces (?)
    return: 
        clean_force: list of all forces as floats
    -----------------------
    just converts whatever type of forces are in the input list to floats (?)
    '''
...
def make_new_g96(
    ...
    '''
    Input:
        propagator: user chosen propagator algorithm, steepest descent, conjugate gradient or bfgs
        xyzq
        new_xyzq
        total_force
        last_forces
        gro
        new_gro
        initstep: current step size I guess
        logfile
    Return: None
    ----------------------
    Writes new g96 file with updated atom positions based on forces and propagator
    '''
    dispvec = []
    maxforce = 0.0
...
def remove_inactive(total_force, active):
    '''
    sets all forces on inactive atoms to zero
    '''
    new_total_force = []
...
def qmmm_prep(
    ...
    input:
        ...
        pathinfo: path information for gmx, Grolib and g16
    returns:
        new_xyzq: new xyzq matrix
        m1list: updated m1 atoms (positions or indices?)
        m2list: updated m2 atoms
        new_links: updated link atoms
    -----------------
    I don't quite understand this function, a lot of pcf stuff that I haven't looked into yet
    '''
...
def make_opt_step(
    ...
    Input:
        ...
        xyzq: Initial coordinates and charges.
        connlist: Connectivity list.
        propagator: Optimization method (STEEP, CONJGRAD, BFGS).
        gro: Input GROMACS .g96 file.
        top: GROMACS topology file.
        total_force: Total forces on atoms.
        initstep: Initial step size for optimization.
        qmatomlist: List of QM atoms.
        qm_corrdata: QM correction data.
        m1list, m2list: Lists of atoms associated with QM region.
        q1list: List of QM atoms and their connectivity.
        qmmmtop: Topology information for QM/MM simulation.
        qminfo, mminfo, qmmminfo: Information about QM, MM, and QM/MM regions.
        linkcorrlist: List of link atom charge correction pairs.
        flaglist: List of flags.
        pcffile: Point charge field file.
        linkatoms: List of link atoms.
        curr_step: Current optimization step.
        last_energy: Energy from the last step.
        last_forces: Forces from the last step.
        logfile: Log file for recording information.
    ...
    Returns:
        ...
        new_links
        initstep
        imporved: boolean if energy improved
    -------------------------------
    makes new step based on the forces and checks if new energy is better or worse, if worse step size is adapted until too small
...
def g96_to_gro(inp, out, logfile):
    '''
    writes the final geometry to a gro file 
    '''
...
def read_pcf_self(qmfile):
    '''
    reads and returns the energy of the point charges
    '''
    pcf_self = 0.0
...
def get_qmenergy(qmfile, qmprog, extra_string, pcffile, logfile, basedir):
    '''
    reads qm energy from qm output and corrects energy with energy of pcf
    '''
...
def get_mmenergy(edrname, realprefix, logfile):
    '''
    calls gmx via subprocess to obtain mm energy, converts and returns mm energy in au
    '''
    mmenergy = 0.0
...
def get_m2charges(xyzq, m1list, m2list):
    '''
    reads and returns m2 charges from xyzq matrix
    '''
    m2charges = []
    count = 0
...
def read_pcffile(pcffile):
    '''
    reads the pcf file and returns a list with coordinates and charges (? for which atoms?)
    '''
    pcf = []
    with open(pcffile) as ifile:
        for line in ifile:
...
def get_linkenergy_au(
    ...
    '''
    calculates (coulomb) and corrects (?) the energy of the link atoms, returns energy
    '''
...
def get_energy(
    ...
    '''
    makes input files and runs them for qm and mm calculations, extracts energy 
    '''
    prefix = pathinfo[3]
    realprefix = prefix + pathinfo[7]
    mmenergy = 0.0
    ...
    logger(logfile, "Test, logger working? MMinfo[1]=\n")
    logger(logfile, str(mminfo[1]))
    ...
    # runs qm file (unless there already is an output)
    if qminfo[0] == "G16":
        g16cmd = pathinfo[4]
...
def read_forces(
    ...
    '''
    reads all types of forces and calculates and returns total force
    '''
    ...
    total_force=remove_inactive(total_force,active) #in SP(Inner/Outer-function) case I don't have inactive atoms
...
def opt_cycle(
    ...
    '''
    checks if the maximum force on any atom is below the user chosen force threshold, if not it calls another opt step until one of the ending criteria is met
    '''
    done = 0
...
def scan_cycle(
    ...
    '''
    creates new g96 file with new coordinates for scan atoms and calls a singlepoint
    '''
    # already run sp once
    ...
    # make new geo with new step displacement?
    with open(new_gro, "w") as ofile:
        with open(gro) as ifile:
            counter = 0
            for line in ifile:
...
def perform_sp(
    ...
    '''
    gets energy and forces of current info and writes forces to output file (where are the subprocesses called though?)
    '''
...
def perform_opt(
    ...
    '''
    gets last energy, runs loop of optimization cycles until one ending criterion is met
    '''
    count = qmmminfo[7]
    qmenergy, mmenergy, qm_corrdata = get_energy(
        gro,
...
def perform_scan(
    ...
    '''
    gets energy and forces, reads scan data (from scan.txt), calls scan cycle
    '''
    # SP
    jobname = stepper(qmmminfo[0], step)
    qmenergy, mmenergy, qm_corrdata = get_energy(
...
def perform_job(
    ...
    '''
    reads jobtype and calls perform_* based on jobtype, passes all arguments to the job, is it ever called? 
    '''
...
def databasecorrection(
    ...
    '''
    seems to be correcting some values based on some database
    '''
    forcefield = mminfo[0]
    method = qminfo[1]
    basisset = qminfo[2]
...
def get_approx_hessian(xyz, old_xyz, grad, old_grad, old_hess, logfile):
    '''
    calculates and returns a new hessian matrix and it's smalles eigenvalue (?)
    '''
    s = xyz - old_xyz
    if s.shape[1] != 1:
